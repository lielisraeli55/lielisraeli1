...
* רצה כ-**Foreground Service** ברקע — האפליקציה לא חייבת להיות פתוחה כדי שהיא תעבוד.
* מקשיבה לחיישן הקרבה של המכשיר.
* כשמשהו מתקרב לחיישן (פחות מ־1.5 ס״מ בערך) — משמיעה את הסאונד שב-`res/raw/sound.mp3`.
* כשמתרחקים — הסאונד מפסיק.
* התראה קבועה ב-status bar עם כפתור "עצור".
* מתחילה אוטומטית אחרי אתחול המכשיר (אם השירות היה דלוק).

## איך מפעילים

1. פותחים את האפליקציה פעם אחת.
2. אם זאת אנדרואיד 13+ — מאשרים את הרשאת ההתראות שמוקפצת.
3. לוחצים "הפעל ברקע".
4. אפשר לסגור את האפליקציה לגמרי — השירות ממשיך לרוץ.
5. כדי לעצור: לוחצים "עצור" בהתראה, או חוזרים לאפליקציה ולוחצים "עצור".
